...
#     `uname -r` outputs the current installed kernel release - we include it in backticks so that the output is includes as part of the command (you might also see
#     this as $(uname -r)
...
read -n 1 -s -p "TAKE CARE! You will be left with only one kernel - the current one! Press a key to continue "
