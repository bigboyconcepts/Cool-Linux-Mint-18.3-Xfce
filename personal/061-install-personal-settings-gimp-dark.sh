...
if [ -d $HOME"/.gimp-2.8" ] && [ -d $HOME"/.gimp-2.8-old" ]; then
    if [ ! -d $HOME"/.gimp-2.8-dark" ]; then
        mv $HOME"/.gimp-2.8" $HOME"/.gimp-2.8-dark"
    else
        rm -fr $HOME"/.gimp-2.8-dark"
        mv $HOME"/.gimp-2.8" $HOME"/.gimp-2.8-dark"
    fi
fi
...
cp $PERSONAL_FOLDER/settings/gimp/gimp-2.8-dark.tar.gz ~
cd $HOME; tar xzf gimp-2.8-dark.tar.gz
rm gimp-2.8-dark.tar.gz
